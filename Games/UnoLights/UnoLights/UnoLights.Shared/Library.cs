...
{
    private const string title = "Uno Lights";
    private const int on = 1;
    private const int off = 0;
    private const int size = 7;
    private readonly Color lightOn = Color.FromArgb(255, 103 ,229, 173);
    private readonly Color lightOff = Color.FromArgb(255, 122, 103, 248);
    ...
    private void Toggle(Grid grid, int row, int column)
    {
        ...
        var button = (Button)grid.FindName($"{row}:{column}");
        button.Background = _board[row, column] == on ? 
