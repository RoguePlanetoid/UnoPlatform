...
public class Library
{
    ...
    {
        Color.FromArgb(255, 103, 229, 173), Color.FromArgb(255, 122, 103, 248),
        Color.FromArgb(255, 248, 89, 119), Color.FromArgb(255, 21, 155, 255)
    };
    ...
    private List<int> Choose(int total) =>
        Enumerable.Range(0, total).OrderBy(r => _random.Next(0, total)).ToList();
    ...
    private Grid Portion(string name, int colour)
    {
        var grid = new Grid()
        {
            Name = name,
            ...
            Background = new SolidColorBrush(colours[colour])
        };
        // Setup Grid
    ...
    private StackPanel Domino(string name, int offset)
    {
        ...
        panel.Children.Add(Portion($"{name}.{name_upper}", offset));
        panel.Children.Add(Portion($"{name}.{name_lower}", offset + 1));
        return panel;
    }
    private void Layout(Grid grid)
    {
        ...
        panel.Children.Add(Domino(set_one, 0));
        panel.Children.Add(Domino(set_two, 2));
